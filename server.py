    ...
    * self.room: one of 0, 1, 2, 3. This signifies which "room" the client is in,
    ...
    def room_description(self, room_number):
        ...
        return [
            ...
            "You are in the room with the black wallpaper."
        ][room_number]
    ...
    def move(self, argument):
        ...
        * "west"
        ...
        if self.room == 0:
            if argument.lower() == 'north':
                self.room = 3
            if argument.lower() == 'west':
                self.room = 1
            if argument.lower() == 'east':
                self.room = 2

        if self.room == 1 and argument.lower() == 'east':
                self.room = 0

        if self.room == 2 and argument.lower() == 'west':
                self.room = 0

        if self.room == 3 and argument.lower() == 'south':
                self.room = 0
    ...
    def route(self):
        ...
        if received[0] == 'move':
            self.move(received[1])

        if received[0] == 'say':
            self.say(' '.join(received[1:]))
